...
We start by running the code from "Notes on likelihood" from lab 10: https://github.com/eco-evo-thr-2022/10-likelihood/blob/main/likelihood_how-to.Rmd. To make the data look like SADs, we need to sort in decreasing rank abundance and subset the data. Variables for different sampling times are still within each subset.
...
These plots look pretty similar, so for simplicity sake maybe we can analyze them all together for now. Does the logseries equation describe our data well? That could indicate neutral processes.
...
It looks like the lognormal (blue) distribution and the weibull (green) distribution might be the best fit for our data.
